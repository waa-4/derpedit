...
let p={version:'0.4',name:'My Derpedit Game',gameType:'topdown',background:'#202633',startRoom:'room1',music:[],rooms:[{id:'room1',name:'Main Menu',type:'menu',objects:[]},{id:'room2',name:'Level 1',type:'game',objects:[]}],assets:[]},roomId='room1',sel=null,hist=[],future=[],drag=null,resize=null,game=null,runState=null,selectedAsset=null;
...
function normalize(){if(!p.rooms){p.rooms=[{id:'room1',name:'Level 1',type:'game',objects:p.objects||[]}];p.startRoom='room1';delete p.objects}delete p.runtimeInventory;p.assets=p.assets||[];p.music=p.music||[];p.sounds=p.sounds||[];p.variables=p.variables||{};p.multiplayer=p.multiplayer||{enabled:false,url:'',key:'',maxPlayers:8};p.gameType=p.gameType||'topdown';for(const r of p.rooms){r.cameraMode??='fixed';r.width??=1280;r.height??=720;r.infinite??=false;r.musicId??='';r.waveEnabled??=false;r.waves??=[];for(const o of r.objects){o.health??=100;o.damage??=0;o.cooldown??=.5;o.layer??='world';o.script??='';o.action??='none';o.targetRoom??='';o.opacity??=100;o.renderLayer??=0;o.physics??=false;o.gravity??=false;o.collision??=true;o.team??='Neutral';o.animations??={idle:'',walk:'',jump:'',fall:''};o.breakable??=false;o.breakHealth??=100;o.breakDrop??='';
...
$('breakable').checked=!!o.breakable;$('breakHealth').value=o.breakHealth??100;$('breakDrop').value=o.breakDrop||'';$('physics').checked=!!o.physics;$('gravity').checked=!!o.gravity;$('collision').checked=o.collision!==false;
