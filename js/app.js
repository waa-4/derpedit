...
inventory:p.runtimeInventory.inventory,
hotbar:p.runtimeInventory.hotbar,
equipped:p.runtimeInventory.equipped,
projectiles:[],
lastShotAt:0};buildRuntimeButtons();if(window.renderRuntimeHotbar)window.renderRuntimeHotbar(game);requestAnimationFrame(loop)}
...
for(const item of g.O.filter(o=>o.type==='item'&&!o.collected&&!o.destroyed&&o.autoPickup!==false)){
 if(hit(g.pl,item)){
  ...
  p.runtimeInventory={inventory:g.inventory,hotbar:g.hotbar,equipped:g.equipped};
  persistProject();
  if(window.renderRuntimeHotbar)window.renderRuntimeHotbar(g);
  status(itemId+' picked up and equipped.');
 }
}
